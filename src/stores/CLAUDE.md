...
|-------|-----------|-------|
| `authStore` | Yes (localStorage `usdx-auth`) | `user`, `token`, `isAuthenticated`; `setPinSet(bool)` corrects `user.pinSet` alone (USDX-651: `/pin/set` success → true at once; `401 PIN_NOT_SET` → false) — call it through `hooks/usePinSetCorrection`, which also patches the `/auth/me` cache. `patchUser({ pinSet?, twoFactorEnabled? })` is the general form (USDX-714) — written only by `hooks/useProfileCorrection` |
