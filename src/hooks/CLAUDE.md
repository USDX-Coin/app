...
|------|-------|---------------|---------|
| `useAuth` | `authStore` | `useMutation` (login, register, 2FA verify-login + email recovery) | Auth flow + router redirect. Login lands on `/mint`, or on the screen of a pending re-login intent (`lib/auth/relogin-intent`, USDX-697). On a 2FA account `login` resolves `{ twoFactorRequired: true }` and touches neither store nor router; `verifyTwoFactorLogin(code)` is the step that stores the session and lands (same landing, intent included). `requestTwoFactorRecovery` / `confirmTwoFactorRecovery(otp)` = email OTP → 2FA off, no token (the caller logs in again) (USDX-714) |
...
| `useRedeem` | `redeemStore` | `useMutation` (createRedeem) | Redeem form logic, validation. Burn source (USDX-567): `custodial` (PIN + `twoFactorCode` in the body, `burnMode` from the backend, no client burn) or `external` (unchanged self-sign gate). Custodial 2FA (USDX-717) via `useCustodialStepUp`: `twoFactorErrorKey` / `twoFactorCooldownSeconds` stay in the PIN dialog, `twoFactorSetupRequired` / `outboundLockedUntil` / `stepUpBlocked` (all off on the external source) |
| `useCustodialStepUp` | `authStore` (reads `user.twoFactorEnabled`, corrects it via `useProfileCorrection`) | reads/patches the `GET /api/v2/wallet` cache | 2FA guard shared by transfer + custodial redeem (`custodial-wallet.md` §6.1, USDX-717): `twoFactorSetupRequired` (`false` on the profile; `undefined` does not block), `lockedUntil` (GET /wallet `outboundLockedUntil` or `409 CUSTODIAL_OUTBOUND_LOCKED` `details.lockedUntil` — patched into the wallet cache so both hook instances see it; lifts by itself when it passes), `blocked`, the `2fa-stepup` countdown; `onError` does the corrections. Pure helpers: `stepUpErrorKey` (code-field sentence), `isPinLockout` (429 scope `pin` or no scope = backend before 718) |
| `useTransfer` | `transferStore` | `useMutation` (`POST /api/v2/wallet/transfer`) | Custodial transfer: validation, `Idempotency-Key` once per intent, same-key retry on `409 IDEMPOTENCY_KEY_IN_PROGRESS`, error routing to the PIN dialog vs the Ringkasan (USDX-567); `503 NETWORK_CONGESTED` → its own Ringkasan sentence, key kept (USDX-709). `submitWithPin(pin, twoFactorCode)` — the code is not part of the intent, so a new code reuses the key; `mapTransferError` routes 2FA answers to `twoFactor` (code field) or `guard` (card / lock banner via `useCustodialStepUp`, USDX-717). Takes `t` so `mapTransferError` can fill vars. `pinNotSet` is read from the profile copy only; `401 PIN_NOT_SET` corrects it via `usePinSetCorrection(false)` (USDX-651) |
...
| `useProfileCorrection` | `authStore` (`patchUser`) + query cache | — | The one writer of a profile-flag correction — `pinSet` and `twoFactorEnabled` (USDX-714): store AND the `["session","me"]` cache, so reopening a `useSession` screen cannot write a stale cached /auth/me back (USDX-651, `custodial-wallet.md` §5.1) |
| `usePinSetCorrection` | via `useProfileCorrection` | — | `(pinSet) => correct({ pinSet })` for usePin / useTransfer / useRedeem |
| `useTwoFactor` | `authStore` (reads/corrects `user.twoFactorEnabled`) | `useMutation` × 4 (`POST /api/v2/auth/2fa/enable`, `/verify`, `/disable`, `/backup-codes/regenerate`) | 2FA in Settings (USDX-714): `twoFactorEnabled` (`null` = older session without the field — no action offered), one cooldown for the `2fa-verify` lockout (Retry-After), `mapTwoFactorError` routes each answer to `password` / `code` / `form`. verify success → `true`, disable success or `TWO_FACTOR_NOT_ENABLED` → `false`, through `useProfileCorrection` + background /auth/me refresh |
...
| `useTransactions` | — | `useQuery` (`GET /api/v2/transactions`) | Unified /history list (USDX-713): mint + redeem + custodial transfers in/out (`type`, or `includeTransfers` for "Semua"), page/take, `keepPreviousData`. Refreshes every 15 s (`HISTORY_REFRESH_MS`) only while a transfer on screen is PENDING (unknown status counts), stops when none is left or the tab is hidden; 429 → waits `Retry-After` (≤ 2 retries) and the next refresh waits at least that long. `TRANSACTIONS_KEY` is invalidated by `useTransfer` after a broadcast and by the tracker on its first final status, so a fresh cache never hides a new transfer (review app#79) |
