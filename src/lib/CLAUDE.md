...
| `api/mock-wallet-transfers.ts` | Mock transfer history (USDX-701): ledger in localStorage (`usdx-mock-wallet-transfers`), newest first + page/take, 404 for another user's id, the "receipt watcher" decides each new transfer 3.5 s after broadcast (`transferOutcome` seam on the custodial state). Fixtures (three statuses, two failure reasons) live in `api/mock-wallet-transfer-fixtures.ts` — types only, so Playwright can import them |
