...
// which is a build with `env.walletCreateEnabled` ON, like the dev build
// (custodial-wallet.md §1, amandemen 21 Sep 2026, USDX-699; production keeps the
// "Segera hadir" pill, covered by the unit test of CustodialWalletOffer):
//   1. new user registers → verifies → lands on /mint (the verify-email redirect
//      to the wallet step stays OFF in every environment, amandemen 14 Sep) →
//      opens the wallet step → "dikasih wallet" → ACTIVE, balance 0 → receives
//      USDX → the balance shows the real number
...
test.describe("Custodial wallet flow", () => {
  test.describe("positive", () => {
    test("register → verify → dikasih wallet → ACTIVE with 0 → receives USDX → balance shows", async ({
      ...
      // The activation link lands on the dashboard, like login — the redirect to
      // the wallet step is off in every environment. The step itself is one URL away.
