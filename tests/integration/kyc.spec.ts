...
async function fillForm(page: Page) {
  ...
  //
  // Dropdown dijawab lewat `pickKycSelect`: kontrolnya Radix sejak commit 8177459,
  // jadi yang dikirim adalah LABEL yang terlihat, bukan nilai enum (USDX-671).
...
test.describe("KYC Responsive (1440)", () => {
  ...
  test("form renders fully on desktop", async ({ page }) => {
    await gotoKyc(page, "UNVERIFIED");
    await expect(page.getByLabel("First Name")).toBeVisible();
    // `toHaveValue` mati di sini sejak dropdown pindah ke Radix: triggernya
    // `<button>`, bukan `<select>`, dan nilainya tidak pernah jadi atribut `value`
    // (USDX-671). Nilai awal `KTP` tetap dibuktikan — sekarang lewat label yang
    // benar-benar terbaca nasabah di trigger, bukan lewat kode enumnya.
    await expect(page.locator("#identityType")).toHaveText("KTP (Indonesian ID card)");
