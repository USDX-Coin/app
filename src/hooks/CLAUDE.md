...
| `useTransfer` | `transferStore` | `useMutation` (`POST /api/v2/wallet/transfer`) | Custodial transfer: validation, `Idempotency-Key` once per intent, same-key retry on `409 IDEMPOTENCY_KEY_IN_PROGRESS`, error routing to the PIN dialog vs the Ringkasan (USDX-567). Takes `t` so `mapTransferError` can fill vars. `pinNotSet` is read from the profile copy only; `401 PIN_NOT_SET` corrects it via `authStore.setPinSet(false)` (USDX-651) |
| `usePin` | `authStore` (writes `user.pinSet`) | `useMutation` × 2 (`POST /api/v2/auth/pin/set`, `/change`) | Create / change the account PIN (USDX-651): one cooldown for the shared `pin` lockout, `mapPinError` routes each answer to a field (`current` / `new` / `form`), success flips `user.pinSet` to `true` at once and invalidates `/auth/me`; stale copies follow the backend (`REAUTH_REQUIRED` → true, `PIN_NOT_SET` → false) |
