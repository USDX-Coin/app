...
| `usePin` | `authStore` (writes `user.pinSet`) | `useMutation` × 2 (`POST /api/v2/auth/pin/set`, `/change`) | Create / change the account PIN (USDX-651): one cooldown for the shared `pin` lockout, `mapPinError` routes each answer to a field (`current` / `new` / `form`), success flips `user.pinSet` to `true` at once and invalidates `/auth/me`; stale copies follow the backend (`REAUTH_REQUIRED` → true, `PIN_NOT_SET` → false). All corrections go through `usePinSetCorrection` |
